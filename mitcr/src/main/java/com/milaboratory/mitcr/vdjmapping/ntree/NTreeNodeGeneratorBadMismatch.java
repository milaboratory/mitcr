 ...
 */
public class NTreeNodeGeneratorBadMismatch extends AbstractGenerator<NTreeSlider, NucleotideInfo> implements NTreeNodeGenerator {
    public static final NTreeNodeGeneratorBadMismatch INSTANCE = new NTreeNodeGeneratorBadMismatch();

    @Override
    protected void addToNext(NTreeSlider current, List<NTreeSlider> nextGeneration, NucleotideInfo condition) {
        if (condition.bad) {
            for (byte i = 0; i < 4; ++i)
                if (current.canSlide(i))
