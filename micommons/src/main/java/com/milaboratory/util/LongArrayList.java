...
/**
 * Array list of primitive long integers.
 *
...
public final class LongArrayList {
    ...
    private LongArrayList(long[] data, int size) {
        this.data = data;
        this.size = size;
    }
    ...
    public void addAll(LongArrayList longArrayList) {
        int arrLen = longArrayList.size();
        ensureCapacity(arrLen + size);
        System.arraycopy(longArrayList.data, 0, data, size, arrLen);
        size += arrLen;
    }

    public void set(int position, long num) {
        if (position < 0 || position >= size)
            throw new IndexOutOfBoundsException();
        data[position] = num;
    }
    ...
    public boolean replaceFirst(long from, long to) {
    ...
    public boolean replaceAll(long from, long to) {
    ...
    public LongArrayList clone() {
        return new LongArrayList(Arrays.copyOf(data, data.length), size);
    }
